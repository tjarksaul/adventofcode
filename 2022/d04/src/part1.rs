pub fn main() {
    ...
    println!("Amount of partially overlapping pairs: {}", partially_overlapping_pairs);
}

pub type Pair = ((i32, i32), (i32, i32));

fn count_fully_overlapping_pairs(pairs: &Vec<Pair>) -> i32 {
    return pairs.iter().map(is_fully_overlapping_pair).fold(0, |a, b| a + (b as i32));
}
...
fn count_partially_overlapping_pairs(pairs: &Vec<Pair>) -> i32 {
    return pairs.iter().map(is_partially_overlapping_pair).fold(0, |a, b| a + (b as i32));
}
